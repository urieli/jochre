...
public class FeedbackDAO {
	...
	List<FeedbackSuggestion> findUnappliedSuggestions() {
		...
		try {
			...
			sql = "SELECT " + SELECT_SUGGESTION + ", " + SELECT_WORD + ", " + SELECT_ROW + ", " + SELECT_DOCUMENT + " FROM joc_suggestion"
					...
					+ " INNER JOIN joc_document ON row_doc_id = doc_id" + " WHERE suggest_applied=:false" + " AND suggest_ignore=:false"
					+ " ORDER BY suggest_id";
			MapSqlParameterSource paramSource = new MapSqlParameterSource();
	...
	List<FeedbackSuggestion> findSuggestions(FeedbackDocument doc, int pageIndex) {
		...
		try {
			...
			sql = "SELECT " + SELECT_SUGGESTION + ", " + SELECT_WORD + ", " + SELECT_ROW + ", " + SELECT_DOCUMENT + " FROM joc_suggestion"
					...
					+ " INNER JOIN joc_user ON suggest_user_id = user_id" + " INNER JOIN joc_ip ON suggest_ip_id = ip_id"
					+ " INNER JOIN joc_word ON suggest_word_id = word_id" + " INNER JOIN joc_row ON word_row_id = row_id"
					+ " INNER JOIN joc_document ON row_doc_id = doc_id" + " WHERE row_doc_id=:row_doc_id" + " AND row_page_index=:row_page_index"
	...
	Map<Integer, List<FeedbackSuggestion>> findSuggestions(FeedbackDocument doc) {
		...
		try {
			...
			sql = "SELECT " + SELECT_SUGGESTION + ", " + SELECT_WORD + ", " + SELECT_ROW + ", " + SELECT_DOCUMENT + " FROM joc_suggestion"
					...
					+ " INNER JOIN joc_user ON suggest_user_id = user_id" + " INNER JOIN joc_ip ON suggest_ip_id = ip_id"
					+ " INNER JOIN joc_word ON suggest_word_id = word_id" + " INNER JOIN joc_row ON word_row_id = row_id"
					+ " INNER JOIN joc_document ON row_doc_id = doc_id" + " WHERE row_doc_id=:row_doc_id" + " AND suggest_ignore=:false"
	...
	void saveSuggestion(FeedbackSuggestion suggestion) {
		...
		if (suggestion.isNew()) {
			...
			sql = "UPDATE joc_suggestion" + " SET suggest_applied=:suggest_applied" + ", suggest_ignore=:suggest_ignore" + " WHERE suggest_id=:suggest_id";
