...
public class JochreQueryTest {
	...
	public void test() throws IOException {
		...
		try {
			...
			query = new JochreQuery(config, "קײנער", Arrays.asList("שלום עליכם"), false, "", null, null, false);
			results = searcher.search(query, 0, 100);
			assertEquals(0, results.getRight().intValue());

			query = new JochreQuery(config, "קײנער", new ArrayList<>(), false, "", 1917, 1917, false);
			results = searcher.search(query, 0, 100);
			assertEquals(1, results.getRight().intValue());

			query = new JochreQuery(config, "קײנער", new ArrayList<>(), false, "", 1918, 1920, false);
			results = searcher.search(query, 0, 100);
			assertEquals(0, results.getRight().intValue());
