        ...
        [#if !paragraph.junk]
          ...
          [#list paragraph.rows as row]
            ...
            [#list row.groups as group]
              ...
              <String HEIGHT="${group.height?c}" WIDTH="${group.width?c}" HPOS="${group.left?c}" VPOS="${group.top?c}" CONTENT="${group.word?replace("\"", "&quot;")}" WC="${group.confidence?string["0.00"]}">
              [#list group.correctedShapes as shape]
                <Glyph HEIGHT="${shape.height?c}" WIDTH="${shape.width?c}" HPOS="${shape.left?c}" VPOS="${shape.top?c}" CONTENT="${shape.letter?replace("\"", "&quot;")}" GC="${shape.confidence?string["0.00"]}" />
              [/#list]
              </String>
            [/#list]
            </TextLine>
