...
public class Vectorizer {
				...
				for (GroupOfShapes group : row.getGroups()) {
					for (Shape shape : group.getShapes()) {
						...
						for (LineSegment line : lines)
							graphics2D.drawLine(shape.getLeft() + line.getStartX(), shape.getTop() + line.getStartY(), shape.getLeft() + line.getEndX(),
	...
	/**
	 * Get the line segments going from a particular point in the outline to an
	 * opposite edge.
	 */
	List<LineSegment> getLinesToEdge(Shape shape, int xOrigin, int yOrigin, int threshold) {
		...
		for (LineDefinition lineDef : this.getLineDefinitions()) {
			if (LOG.isDebugEnabled()) {
				String lineDefText = "Sector " + lineDef.getSector() + ". Delta (" + lineDef.getDeltaX() + "," + lineDef.getDeltaY() + "). " + "Incr ("
						+ lineDef.getXIncrement() + "," + lineDef.getYIncrement() + "). ";
				lineDefText += "{";
			...
			if (length != 0) {
				LineSegment lineSegment = new LineSegment(shape, lineDef, xOrigin, yOrigin, endX, endY);
				...
				if (!foundLineSegment) {
					if (LOG.isDebugEnabled())
						LOG.debug("Line (" + lineSegment.getStartX() + "," + lineSegment.getStartY() + ") " + "(" + lineSegment.getEndX() + "," + lineSegment.getEndY()
	 ...
	 * inside 45° of a circle. These go from a straight line (infinity) to a
	 * diagonal line (1). The numbers represent the steps to take before taking
	 * one step in the other axis. Thus, {1} means take 1 step in the x axis, 1
	 * step in the y axis {2} means take 2 x-steps followed by 1 y-step, and start
	 * over {2,1} means take 2 x-steps, 1 y-step, 1 x-step, 1 y-step and start
	 * over {0} represents infinity = stay in the original axis.
	 */
		...
		if (this.lineDefinitions == null) {
			...
			for (int sector = 0; sector < 4; sector++) {
				for (List<Integer> steps : stepList) {
					LineDefinition lineDef = new LineDefinition(sector, index++);
	...
	private List<LineSegment> combineSegments(Shape shape, List<LineSegment> lineSegments) {
		...
		for (int i = 0; i < lineSegments.size() - 1; i++) {
			for (int j = i + 1; j < lineSegments.size(); j++) {
				...
				// is overlap possible?
				if (line1left <= line2right && line1right >= line2left && line1top <= line2bottom && line1bottom >= line2top) {
					// note: line1 is guaranteed to be longer than or of equal length to
					// line 2
		...
		// TODO: combine lines that are "more or less" in the same location &
		// direction
