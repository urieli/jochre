...
/* Original license notice below, from OpenJDK 6-b14, java.awt.image.RescaleOp
...
/**
 * Similar to {@link RescaleOp}, but applies the following formula instead
 * 
 * <pre>
 *for each pixel from Source object {
 *    for each band/component of the pixel {
 *        dstElement = ((srcElement+offset-128)*scaleFactor) + 128
 *    }
 *}
 * </pre>
 * <p>
 * The idea is to get rid of noise on printed text pages, by maximising the
 * contrast after having brightened the image.
 */
public class BrightnessContrastOp implements BufferedImageOp, RasterOp {
	...
	/**
	 * Constructs a new RescaleOp with the desired scale factors and offsets. The
	 * length of the scaleFactor and offset arrays must meet the restrictions stated
	 * in the class comments above. The RenderingHints argument may be null.
	 * 
	 * @param scaleFactors
	 ...
	 * @param hints
	 *            the specified <code>RenderingHints</code>, or <code>null</code>
	 */
	public BrightnessContrastOp(float[] scaleFactors, float[] offsets, RenderingHints hints) {
	 ...
	 * scaleFactor and offset will be applied to all bands in a source Raster and to
	 * all color (but not alpha) components in a BufferedImage. The RenderingHints
	 ...
	 *            the specified <code>RenderingHints</code>, or <code>null</code>
	 */
	public BrightnessContrastOp(float scaleFactor, float offset, RenderingHints hints) {
		length = 1;
	...
	/**
	 * Returns the scale factors in the given array. The array is also returned for
	 * convenience. If scaleFactors is null, a new array will be allocated.
	 * 
	 * @param scaleFactors
	 ...
	 * 
	 * @param offsets
	 *            the array to contain the offsets of this <code>RescaleOp</code>
	...
	/**
	 * Creates a ByteLookupTable to implement the rescale. The table may have either
	 * a SHORT or BYTE input.
	 * 
	 * @param nElems
	 *            Number of elements the table is to have. This will generally be
	 *            256 for byte and 65536 for short.
	 */
	private ByteLookupTable createByteLut(float scale[], float off[], int nBands, int nElems) {
		...
		for (int band = 0; band < scale.length; band++) {
			...
			for (int i = 0; i < nElems; i++) {
				int val = (int) ((i + bandOff - 128) * bandScale) + 128;
				if ((val & 0xffffff00) != 0) {
					if (val < 0) {
						val = 0;
					} else {
	 ...
	 * byte or short type. The src must be less than 16 bits. All source band sizes
	 * must be the same and all dst band sizes must be the same.
	 */
	...
	/**
	 * Rescales the source BufferedImage. If the color model in the source image is
	 * not the same as that in the destination image, the pixels will be converted
	 * in the destination. If the destination image is null, a BufferedImage will be
	 * created with the source ColorModel. An IllegalArgumentException may be thrown
	 * if the number of scaling factors/offsets in this object does not meet the
	 * restrictions stated in the class comments above, or if the source image has
	 * an IndexColorModel.
	 * 
	 * @param src
	 *            the <code>BufferedImage</code> to be filtered
	 * @param dst
	 *            the destination for the filtering operation or <code>null</code>
	 * @return the filtered <code>BufferedImage</code>.
	 * @throws IllegalArgumentException
	 *             if the <code>ColorModel</code> of <code>src</code> is an
	 *             <code>IndexColorModel</code>, or if the number of scaling factors
	 *             and offsets in this <code>RescaleOp</code> do not meet the
	...
	/**
	 * Rescales the pixel data in the source Raster. If the destination Raster is
	 * null, a new Raster will be created. The source and destination must have the
	 * same number of bands. Otherwise, an IllegalArgumentException is thrown. Note
	 * that the number of scaling factors/offsets in this object must meet the
	 * restrictions stated in the class comments above. Otherwise, an
	 ...
	 *             number of bands, or if the number of scaling factors and offsets
	 *             in this <code>RescaleOp</code> do not meet the requirements
			...
			for (int y = 0; y < height; y++, sY++, dY++) {
				...
				for (int x = 0; x < width; x++, sX++, dX++) {
					...
					for (int z = 0; z < numBands; z++, tidx += step) {
						val = (int) ((srcPix[z] + offsets[tidx] - 128) * scaleFactors[tidx]) + 128;
						// Clamp
						if ((val & dstMask[z]) != 0) {
							if (val < 0) {
								val = 0;
	...
	/**
	 * Returns the bounding box of the rescaled destination image. Since this is not
	...
	/**
	 * Returns the bounding box of the rescaled destination Raster. Since this is
	...
	/**
	 * Creates a zeroed-destination <code>Raster</code> with the correct size and
	 * number of bands, given this source.
	 * 
	 * @param src
	 ...
	 * dstPt is non-null, it will be used to hold the return value. Since this is
	 * not a geometric operation, the srcPt will equal the dstPt.
	 * 
	 * @param srcPt
