...
class ShapeImpl extends EntityImpl implements ShapeInternal {
			...
			for (int y = 0; y < this.getHeight(); y++) {
				for (int x = 0; x < this.getWidth(); x++) {
					...
					if (!black)
						...
						boolean innerPixel =
							this.isPixelBlack(x-1, y, threshold) &&
							this.isPixelBlack(x+1, y, threshold) &&
	...
	public int[] getVerticalCounts() {
		if (this.verticalCounts==null) {
			...
			for (int x = 0; x < this.getWidth(); x++) {
				for (int y = 0; y < this.getHeight(); y++) {
					int brightness = this.getPixel(x, y);
					if (brightness < 0) brightness = 256 + brightness;
	...
	public String toString() {
		return "Shape, left(" + this.getLeft() + ")"
			+ ", top(" + this.getTop() + ")"
			+ ", right(" + this.getRight() + ")"
			+ ", bot(" + this.getBottom() + ")"
			+ " [id=" + this.getId() + "]";
	...
	public Collection<BridgeCandidate> getBridgeCandidates(double maxBridgeWidth) {
		if (this.bridgeCandidates==null) {
			...
			for (VerticalLineSegment inverseLine : inverseLines) {
				if (inverseLine.yTop==0||inverseLine.x==0||
						inverseLine.yBottom==this.getHeight()-1||inverseLine.x==this.getWidth()-1)
			...
			if (candidateList.size()>0) {
				...
				if (LOG.isDebugEnabled()) {
					...
					for (VerticalLineGroup aGroup : groups) {
						LOG.debug("Group " + i++ + ", pixelCount: " + aGroup.pixelCount + ", leftBoundary: " + aGroup.leftBoundary + ", rightBoundary: " + aGroup.rightBoundary);
					...
					for (BridgeCandidate candidate : candidateList) {
						LOG.debug("Candidate x = " + candidate.x + ", top = " + candidate.yTop + ", bottom = " + candidate.yBottom);
						LOG.debug("- Left group = pixelCount: " + candidate.leftGroup.pixelCount + ", leftBoundary: " + candidate.leftGroup.leftBoundary + ", rightBoundary: " + candidate.leftGroup.rightBoundary);
						LOG.debug("- Right group = pixelCount: " + candidate.rightGroup.pixelCount + ", leftBoundary: " + candidate.rightGroup.leftBoundary + ", rightBoundary: " + candidate.rightGroup.rightBoundary);
					}
				} // should we log?
