...
class SegmenterImpl implements Segmenter {
	...
	private SourceImage sourceImage = null;
	...
	public SegmenterImpl(SourceImage sourceImage) {
		this.sourceImage = sourceImage;
	}

	/* (non-Javadoc)
	 * @see com.joliciel.ochre.graphics.ShapeExtractor#getLines(com.joliciel.ochre.graphics.ImageGrid)
	 */
	public void segment() {
		LOG.debug("########## segment #########");
	...
	void removeOversizedShapes(List<Shape> shapes) {
		...
		for (Shape largeShape : largeShapes) {
			...
			if (ratio<=maxFrameCardinalityRatio) {
				...
				List<Rectangle> whiteAreas = whiteAreaFinder.getWhiteAreas(mirror, 0, 0, 0, mirror.getWidth()-1, mirror.getHeight()-1, minWhiteAreaWidth, minWhiteAreaHeight);
				int whiteAreaSize = 0;
				for (Rectangle whiteArea : whiteAreas) {
		...
		for (int y = 0; y < sourceImage.getHeight(); y++) {
			for (int x = 0; x < sourceImage.getWidth(); x++) {
				if (sourceImage.isPixelBlack(x, y, sourceImage.getSeparationThreshold())) {
					...
					shapes.add(shape);
					//List<Shape> splitShapes = this.getShapes(sourceImage, mirror, x, y);
...
//						}
			...
			for (Shape otherShape : shapes) {
				...
				if (!shape.getRow().equals(otherShape.getRow())&&!otherShape.equals(shape)) {
					...
					if (otherShape.getRight()> newSearchLeft && otherShape.getRight() < shape.getLeft()
							&& otherShape.getTop() <= shape.getBottom() && otherShape.getBottom() >= shape.getTop()) {
		...
		for (Set<Shape> speckCluster : speckClusters) {
			...
			if (useWidth)
				scale = speckWidth < maxSpeckWidthFloor ? 0.0 :
					...
					((double) speckWidth - maxSpeckWidthFloor) / (maxSpeckWidthCeiling - maxSpeckWidthFloor));
			else
			...
			for (Shape otherShape : shapes) {
				...
				if (!specks.contains(otherShape)) {
					...
					if (otherShape.getTop() <= clusterBottom && otherShape.getBottom() >= clusterTop) {
						...
						int nearestTop = (otherShape.getTop()>otherShape.getTop()+otherShape.getMeanLine()) ? otherShape.getTop()+otherShape.getMeanLine() : otherShape.getTop();
						int nearestBot = (otherShape.getBottom()<otherShape.getTop()+otherShape.getBaseLine()) ? otherShape.getTop()+otherShape.getBaseLine() : otherShape.getBottom();
			...
			if (nearestShape!=null) {
				LOG.trace("Nearest shape, top(" + nearestShape.getTop() + ") "
						+ "left(" + nearestShape.getLeft() + ") "
						+ "bot(" + nearestShape.getBottom() + ") "
			...
			if (nearestShape==null)
				...
				for (Shape speck : speckCluster) {
					...
					if (nearestShape.getTop() <= speck.getBottom() && nearestShape.getBottom() >= speck.getTop()) {
						...
						int nearestTop = (nearestShape.getTop()>nearestShape.getTop()+nearestShape.getMeanLine()) ? nearestShape.getTop()+nearestShape.getMeanLine() : nearestShape.getTop();
						int nearestBot = (nearestShape.getBottom()<nearestShape.getTop()+nearestShape.getBaseLine()) ? nearestShape.getTop()+nearestShape.getBaseLine() : nearestShape.getBottom();
	...
	void removeOrphans(SourceImage sourceImage, boolean hasGroups) {
			...
			for (RowOfShapes row : sourceImage.getRows()) {
				for (GroupOfShapes group : row.getGroups()) {
					for (Shape shape : group.getShapes()) {
						if ((shape.getWidth() < maxSpeckWidth && shape.getHeight() < maxSpeckHeight)
							|| (shape.getWidth()<maxSpeckWidthAlone)){
							LOG.debug("Removing shape: " + shape);
		...
		if (hasGroups) {
			...
			for (RowOfShapes row : sourceImage.getRows()) {
				if (row.getBottom() - row.getTop() < minRowHeight
						&& row.getRight() - row.getLeft() < minRowWidth)
				{
					rowsToDelete.add(row);	
				} else if (row.getBottom() - row.getTop() < minWideRowHeight
						&& row.getRight() - row.getLeft() < minWideRowWidth)
				{
					rowsToDelete.add(row);	
				} else {
					boolean hasNonSpeck = false;
	...
	/**
	 * If any two shapes in the same line take up the same horizontal space, we can join them vertically
	 * @param sourceImage
	 */
	...
	/**
	 * If any two shapes in this row take up the same horizontal space, we can join them vertically
	 */
...
//				LOG.debug("Checking " + shape);
				if (shape.getHeight() < maxSpeckHeight && shape.getWidth() < maxSpeckWidth) {
					// only join other shapes to normal height shapes, not to specks
					i++;
					continue;
				}
				int j = 0;
...
//						LOG.debug("Found overlap between " + shape + " and " + otherShape);
						...
						if (intersectionMultiplied >= shape.getWidth()||
								intersectionMultiplied >= otherShape.getWidth()) {
							LOG.debug("Combining " + shape);
	 ...
	 * Returns a List of SimpleRegression representing the centerline for each of the rows.
	 */
	...
	/**
	 * If a row begins with a larger font and then suddenly reduces (e.g. dictionary entries),
	 * we split it into two separate rows and recalculate guidelines for each.
	 * @param sourceImage
	 */
				...
				for (RowOfShapes otherRow : rows) {
					if (!rowsToDelete.contains(otherRow)&&!(currentRow.equals(otherRow))) {
						...
						if (Math.abs(currentRow.getBaseLineMiddlePoint() - otherRow.getBaseLineMiddlePoint())<(2.0*maxRowHeight)
								&& (currentRow.getRight()>=otherRow.getLeft())
							...
							if (currentRow.getBaseLineMiddlePoint() < otherRow.getBaseLineMiddlePoint()) {
								...
								LOG.trace("(otherRow.baseLineMiddlePoint() " + otherRow.getBaseLineMiddlePoint() + " - otherRow.getXHeightMax() " + otherRow.getXHeightMax() + ") - currentRow.baseLineMiddlePoint() " + currentRow.getBaseLineMiddlePoint()); 
							} else {
								distance = (currentRow.getBaseLineMiddlePoint() - currentRow.getXHeightMax()) - otherRow.getBaseLineMiddlePoint();
								LOG.trace("(currentRow.baseLineMiddlePoint() " + currentRow.getBaseLineMiddlePoint() + " - currentRow.getXHeightMax() " + currentRow.getXHeightMax() + ") - otherRow.baseLineMiddlePoint() " + otherRow.getBaseLineMiddlePoint()); 
							}
...
//					}
				...
				if (row.getShapes().size()<=maxShapesForStandaloneParagraph) {
					...
					if (drawSegmentation) {
						...
						if (previousPointStartMargin!=null) {
							...
							graphics2D.drawLine((int) Math.round(previousPointStartMargin.getX()), (int) Math.round(previousPointStartMargin.getY()), (int) Math.round(currentPointStartMargin.getX()), (int) Math.round(currentPointStartMargin.getY()));							
							graphics2D.drawLine((int) Math.round(previousPointEndMargin.getX()), (int) Math.round(previousPointEndMargin.getY()), (int) Math.round(currentPointEndMargin.getX()), (int) Math.round(currentPointEndMargin.getY()));							
							
							graphics2D.setPaint(Color.RED);
							graphics2D.drawLine((int) Math.round(previousPointStartTab.getX()), (int) Math.round(previousPointStartTab.getY()), (int) Math.round(currentPointStartTab.getX()), (int) Math.round(currentPointStartTab.getY()));							
		...
		for (Rectangle whiteArea : sourceImage.getWhiteAreasAroundLargeShapes()) {
			if (whiteArea.getTop()<=row.getBottom()&&whiteArea.getBottom()>=row.getTop()) {
				if (whiteArea.getLeft() - row.getXAdjustment() < leftMargin && whiteArea.getRight() - row.getXAdjustment() > leftMargin) {
					...
					LOG.debug("Overlaps large shape (" + whiteArea.getLeft() + "," + whiteArea.getTop() + "," + whiteArea.getRight() + "," + whiteArea.getBottom() + ")" + " on left by " + overlap);
				}
			}
		}
		return overlap;
		...
		for (Rectangle whiteArea : sourceImage.getWhiteAreasAroundLargeShapes()) {
			if (whiteArea.getTop()<=row.getBottom()&&whiteArea.getBottom()>=row.getTop()) {
				if (whiteArea.getLeft() - row.getXAdjustment() < rightMargin && whiteArea.getRight() - row.getXAdjustment() >rightMargin) {
					...
					LOG.debug("Overlaps large shape (" + whiteArea.getLeft() + "," + whiteArea.getTop() + "," + whiteArea.getRight() + "," + whiteArea.getBottom() + ")" + " on right by " + overlap);
				}
			}
		}
		return overlap;
	...
	private static final class Column extends ArrayList<RowOfShapes> implements Comparable<Column> {
		...
		public String toString() {
			return "Column [adjustedLeft=" + (int) Math.round(adjustedLeft) + ", adjustedRight="
					+ (int) Math.round(adjustedRight) + ", top=" + top + ", bottom=" + bottom
					+ "]";
		}
	...
	void drawSegmentation(SourceImage sourceImage) {
		...
		for (Paragraph paragraph : sourceImage.getParagraphs()) {
				...
				for (GroupOfShapes group : row.getGroups()) {
					...
					if (lastBaseLine!=null) {
						...
						if (xHeight == lastXHeight) {
							if (sourceImage.isLeftToRight()) {
								graphics2D.drawLine(group.getMeanLine()[2], group.getMeanLine()[3],
										lastMeanLine[0], lastMeanLine[1]);
								graphics2D.drawLine(group.getBaseLine()[2], group.getBaseLine()[3],
										lastBaseLine[0], lastBaseLine[1]);
							} else {
								graphics2D.drawLine(group.getMeanLine()[0], group.getMeanLine()[1],
										lastMeanLine[2], lastMeanLine[3]);
								graphics2D.drawLine(group.getBaseLine()[0], group.getBaseLine()[1],
										lastBaseLine[2], lastBaseLine[3]);
							}
						}
					}
					graphics2D.drawLine(group.getMeanLine()[0], group.getMeanLine()[1],
							group.getMeanLine()[2], group.getMeanLine()[3]);
					graphics2D.drawLine(group.getBaseLine()[0], group.getBaseLine()[1],
							group.getBaseLine()[2], group.getBaseLine()[3]);
			...
			graphics2D.drawRect(paragraph.getLeft()-2, paragraph.getTop()-2, paragraph.getRight()-paragraph.getLeft()+4, paragraph.getBottom()-paragraph.getTop()+4);
		} // next paragraph
	}

	Shape getShape(SourceImage sourceImage, WritableImageGrid mirror, int x, int y) {
		Shape shape = this.graphicsService.getDot(sourceImage, x, y);
			...
			if (candidates.size()>0) {
				...
				for (Shape shape : sample) {
					...
					for (LineSegment vector : vectors) {
						List<Integer> vectorThickness = vector.getLineDefinition().findArrayListThickness(shape, vector.getStartX(), vector.getStartY(), vector.getLength(), sourceImage.getSeparationThreshold(), 0, sampleStep);
				...
				for (Shape candidate : candidates) {
					...
					if (candidate.getWidth()>widthForSplittingUpper)
						...
						maxBridgeWidth = (int) Math.ceil(maxBridgeWidthLower +
								(((double) candidate.getWidth() - widthForSplittingLower) / (widthForSplittingUpper - widthForSplittingLower)
		...
		if (bridgeCandidates.size()>0) {
			...
			for (BridgeCandidate bridge : bridgeCandidates) {
				...
				while (!groupStack.isEmpty()) {
					...
					LOG.debug("Touching group, pixelCount: " + lineGroup.pixelCount + ", leftBoundary: " + lineGroup.leftBoundary + ", rightBoundary: " + lineGroup.rightBoundary);
	...
	/**
	 * Should we split and join shapes initially or not.
	 * If not, it is assumed a machine learning algorithm will do this later for us.
	 * @return
	 */
